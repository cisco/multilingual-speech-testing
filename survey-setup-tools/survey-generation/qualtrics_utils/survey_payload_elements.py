...
"""
"""
Qualtrics survey payload utilities.
"""
...
def get_survey_block_element_payload_map(survey_data):
    ...
    block_index_map = {}
    for index, row in survey_data["SurveyElements"][
...
def add_new_survey_block_element_payload(survey_data, block_template):
    ...
    survey_data["SurveyElements"][component_index_survey_map["Survey Blocks"]][
        "Payload"
    ][next_survey_block_element] = block_template

    return survey_data


def insert_new_survey_block_element_payload(
    survey_data, survey_block_id, block_template
):
    ...
    if (
        survey_block_id
        in survey_data["SurveyElements"][component_index_survey_map["Survey Blocks"]][
            "Payload"
        ].keys()
    ):
        survey_data["SurveyElements"][component_index_survey_map["Survey Blocks"]][
            "Payload"
        ][survey_block_id] = block_template
    else:
        last_survey_block_element = get_last_survey_block_element_payload(survey_data)
        next_survey_block_element = str(int(last_survey_block_element) + 1)
