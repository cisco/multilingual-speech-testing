...
def get_current_number_of_questions(survey_data, component_index_survey_map):
    ...
    return int(survey_data['SurveyElements'][component_index_survey_map['Survey Question Count']]['SecondaryAttribute'])
    

    
def update_survey_number_of_questions(survey_data, component_index_survey_map, number_of_questions):
    ...
    survey_data['SurveyElements'][component_index_survey_map['Survey Question Count']]['SecondaryAttribute'] = str(number_of_questions)
    return survey_data
...
def insert_question_referecences_on_block(survey_data, QID_list, payload_index):
    ...
    survey_data['SurveyElements'][0]['Payload'][payload_index]['BlockElements'] = new_question_ref_list
    
    # add question references to the Options of the blokc, Randomization
    survey_data['SurveyElements'][0]['Payload'][payload_index]['Options']['BlockVisibility'] = "Collapsed"
    
    randomized_list = ["{~Randomized~}" for item in QID_list]
    question_id_list = [question_id for question_id in QID_list]
    survey_data['SurveyElements'][0]['Payload'][payload_index]['Options']['Randomization']['Advanced']['FixedOrder'] = randomized_list
    survey_data['SurveyElements'][0]['Payload'][payload_index]['Options']['Randomization']['Advanced']['RandomizeAll'] = question_id_list
