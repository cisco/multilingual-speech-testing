...
def update_survey_number_of_flow_ids(survey_data, number_of_flow_ids):
    ...
    TODO: update this method to compute component_index_survey_map with survey_data
    """
        ...
        if "Flow" in flow_block:
            for item in flow_block["Flow"]:
                if "ID" in item.keys():
                    if block_id == item["ID"]:
                        return i
        elif "ID" in flow_block:
            if block_id == flow_block["ID"]:
                return i
...
def update_survey_embedded_data_fields(survey_data, embedded_data_field, new_value):
    """Update survey embedded data field. This is the embedded data field presented in the
    survey flow.

    Args:
        survey_data(dict): JSON input QSF qualtrics file.
        embedded_data_field (string): name of the embedded data field in qualtrics.
        new_value(string): New value of the embedded data field
    ...
    embedded_field["Value"] = str(new_value)

    survey_data["SurveyElements"][survey_component_index_map["Survey Flow"]]["Payload"][
        "Flow"
    ][0]["EmbeddedData"][index_embedded_data[embedded_data_field]] = embedded_field
    return survey_data


def create_or_update_survey_embedded_data_field(
    survey_data, embedded_data_field, new_value
):
    """Create a new embedded data field in the survey. The embedded data field is created in the
    first flow item in the flow of the survey.

    Update embedded field if exits.
    ...
    if embedded_data_field in index_embedded_data.keys():
        update_survey_embedded_data_fields(survey_data, embedded_data_field, new_value)
    else:
        embedded_data_template = {
            "Description": embedded_data_field,
            "Type": "Custom",
            "Field": embedded_data_field,
            "VariableType": "String",
            "DataVisibility": [],
            "AnalyzeText": False,
            "Value": str(new_value),
        ...
        # Add Embedded data field into the Survey Flow payload
        survey_data["SurveyElements"][survey_component_index_map["Survey Flow"]][
            "Payload"
        ]["Flow"][0]["EmbeddedData"].append(embedded_data_template)
